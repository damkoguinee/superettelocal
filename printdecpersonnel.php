...
<page backtop="10mm" backleft="10mm" backright="10mm" backbottom="10mm" footer="page;">
  ...
  <?php require 'headerticketclient.php';
    ...
    $array = $DB->querys("SELECT max(id) FROM decpersonel "); 
    $Numv=$array['max(id)'];

    $payement = $DB->querys('SELECT numdec, montant, payement, coment, DATE_FORMAT(date_payement, \'%d/%m/%Y \à %H:%i:%s\')AS DateV FROM decpersonel WHERE decpersonel.id= ?', array($Numv));?>
