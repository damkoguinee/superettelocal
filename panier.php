<div class="tableau"><?php
	...
	if ($_SESSION['etab']=="SUPERETTE SHELL TANNERIE CORNICHE") {

		$_SESSION['clientvip']=3; //client journaliers
		unset($_SESSION['clientvipcash']);

	}else{

		$_SESSION['clientvip']=2; //client journalier
		unset($_SESSION['clientvipcash']);
				...
				<form id='monform' method="POST" action="index.php">
					...
					<tbody><?php
						...
						if ($_SESSION['level']<=5) {?>
							...
							<td class="pricec" style="font-size: 25px;"><input style="color:<?=$color;?>; font-size:25px;" type="hidden" min="0" name="pvente" value="<?=number_format($product->pvente,0,',',' ');?>" onchange="this.form.submit()" ><?=number_format($product->pvente,0,',',' ');?></td><?php
				...
				<tr>
					...
					if (isset($_POST['gnfpaye']) or isset($_POST['europaye']) or isset($_POST['uspaye']) or isset($_POST['cfapaye'])){
						...
						if ($_SESSION['level']<=6) {?>
							...
							<th style="background-color: maroon; color: white; font-size: 35px;">TOTAL <label class="totalhaut"><?=" ". number_format(($totcomd),0,',',' '); ?></label></th><?php
			...
			<table style="margin-top: -10px;" class="payement">
				...
				<tbody>
					<form id='remise' method="POST" action="index.php">
						...
						<input style="font-size: 25px; font-weight: bold; width: 92%;" type="hidden" min="0" onchange="this.form.submit()" name="uspaye" value="<?=number_format($prodvente['montantpus'],0,',',' ');?>">

						<input style="font-size: 25px; font-weight: bold; width: 92%;" type="hidden" min="0" onchange="this.form.submit()" name="cfapaye" value="<?=number_format($prodvente['montantpcfa'],0,',',' ');?>">
						...
						<tr><?php 
							...
							if ($_SESSION['level']<=7) {?>
								...
								<input style="font-size: 25px; font-weight: bold; width: 92%;" type="hidden" min="0" onchange="this.form.submit()" name="remise" value="<?=number_format($prodvente['remise'],0,',',' ');?>">
								...
								<td colspan="2" style="width: 15%;"><input style="font-size: 25px; font-weight: bold; width: 92%;" type="text" name="virement" value="<?=number_format($prodvente['virement'],0,',',' ');?>" onchange="this.form.submit()"></td>

								<td colspan="1" style="width: 15%;"><?php 
								...
								<td colspan="2"><input style="font-size: 25px; font-weight: bold; width: 92%;" type="text" name="virement" value="<?=number_format($prodvente['virement'],0,',',' ');?>" onchange="this.form.submit()"></td>
					...
					<tbody>
						...
						<tr>
							...
							<input type="hidden" name="numcheque" value="<?=$prodvente['numcheque'];?>" placeholder="N° du Chèque" onchange="this.form.submit()" style="width: 35%; font-size:15px; font-weight: bold;"/><?php if (!empty($_SESSION['alerteschequep']) and empty($prodvente['numcheque'])) {?><div style="color:red;"><?=$_SESSION['alerteschequep'];?></div><?php }?>

							<input type="hidden" name="banqcheque" onchange="this.form.submit()" style="width: 25%;"/>
								
							<input type="hidden" name="cheque" value="<?=number_format($prodvente['cheque'],0,',',' ');?>" placeholder="montant du Chèque"  onchange="this.form.submit()" style="width: 30%; font-size:25px; font-weight: bold;"> 
					...
					<form id='payement' method="POST" action="payement.php"><?php
						...
						if (empty($panier->totalsaisie()) AND $panier->licence()!="expiree") {?>
							...
							<tr>
		                        	...
		                        	<td><input type="date" name="datev"></td>
		                            ...
		                            <td colspan="2" style="width:20%;"><select type="text" name="stockd">
		                        	...
		                        	<input style="font-size:25px; font-weight: bold;width: 92%;"  type="hidden" min="0" name="fraisup">
		                        	<input style="height: 25px; width: 95%;" type="hidden" name="dateal"><?php

	                            if (empty($_SESSION['clientvip']) and empty($_SESSION['clientvipcash'])) {
	                            }else{?>

	                            		<input type="hidden" name="reste" value="<?=$panier->total();?>"><?php

										if (!empty($_SESSION['clientvipcash'])) {

											if ($total<=0) {?>
												
												<td colspan="2"><input style="cursor: pointer;"  type="submit" name="payer" value="Valider"></td><?php
											}
