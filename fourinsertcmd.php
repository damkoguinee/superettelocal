...
$prodfact = $DB->querys('SELECT max(id) AS max_id FROM fourfacture ');

$numerof=$date + $prodfact['max_id'] + 1; //automatique



$numfact=$numerof;
$numero_commande=$numerof;
...
if (isset($_POST['payer'])){
    ...
    if (isset($prixreel) AND $prixreel!="") {                           
        ...
        $DB->insert('INSERT INTO fourfacture (numcmd, numfact, datefact, fournisseur, taux, montantht, montantva, montantpaye, frais, modep, lieuvente, etatcf, datecmd) VALUES(?, ?, now(), ?, ?, ?, ?, ?, ?, ?, ?, ?, now())', array('cmdf'.$numero_commande, $numfact, $fournisseur, $_SESSION['taux'], $prixreel, 0, $_POST['montantc'], $fraistotaux, $_SESSION['devise'], $lieuvente, $etatc));
